...
export const TimelineSection: React.FC<TimelineSectionProps> = ({
  ...
  return (
    <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3.5 shadow-lg text-slate-200 space-y-3">
      {/* Date Bar & Template Quick Deploy */}
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-2 border-b border-slate-800 pb-3">
        {/* Date Selector */}
        <div className="flex items-center gap-1.5 font-mono">
          <button
            onClick={handlePrevDay}
            className="p-1 rounded bg-slate-950 border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-slate-200 transition-all"
            title="PREV_DAY"
          >
          ...
          <button
            onClick={handleToday}
            className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-[10px] font-bold text-blue-400 border border-slate-700 uppercase"
          >
            TODAY
          </button>
        </div>
        ...
        {/* Template Deploy Hub */}
        <div className="flex items-center gap-1.5 font-mono">
          <span className="text-[10px] text-slate-500 uppercase font-bold hidden sm:inline">TEMPLATE:</span>
          <button
            onClick={() => onDeployTemplate('phase1')}
            className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 text-[10px] font-bold uppercase flex items-center gap-1 transition-all"
          ...
          <button
            ...
            className="px-2.5 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold uppercase flex items-center gap-1 shadow-sm transition-all ml-auto"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 font-mono">
        <div className="bg-slate-950/80 border border-slate-800 rounded p-2 text-center">
          <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">TARGET_LOAD</span>
          <span className="text-sm font-bold text-slate-200">
            {(totalStudyMinutes / 60).toFixed(1)} <span className="text-[10px] text-slate-500">HRS</span>
          </span>
        </div>

        <div className="bg-slate-950/80 border border-slate-800 rounded p-2 text-center">
          <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">COMPLETED</span>
          <span className="text-sm font-bold text-emerald-400">
            {(completedStudyMinutes / 60).toFixed(1)} <span className="text-[10px] text-slate-500">HRS</span>
          </span>
        </div>

        <div className="bg-slate-950/80 border border-slate-800 rounded p-2 text-center">
          <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">REMAINING</span>
          <span className="text-sm font-bold text-amber-400">
            {(remainingStudyMinutes / 60).toFixed(1)} <span className="text-[10px] text-slate-500">HRS</span>
          </span>
        </div>

        <div className="bg-slate-950/80 border border-slate-800 rounded p-2 text-center">
          <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">COMPLETION_RATE</span>
              ...
              >
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-1 font-mono flex-shrink-0">
                    <span className="font-bold text-[11px] text-slate-300 bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800">
                      {task.time}
                    </span>
                ...
                <div className="flex items-center gap-0.5 opacity-40 group-hover:opacity-100 transition-opacity ml-2">
                  <button
